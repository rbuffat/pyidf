...
class TestAvailabilityManagerLowTemperatureTurnOn(unittest.TestCase):
    ...
    def test_create_availabilitymanagerlowtemperatureturnon(self):

        pyidf.validation_level = ValidationLevel.error

        obj = AvailabilityManagerLowTemperatureTurnOn()
        # alpha
        var_name = "Name"
        obj.name = var_name
        # node
        ...
        self.assertEqual(idf2.availabilitymanagerlowtemperatureturnons[0].sensor_node_name, var_sensor_node_name)
        self.assertAlmostEqual(idf2.availabilitymanagerlowtemperatureturnons[0].temperature, var_temperature)