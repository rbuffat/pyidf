...
class ZoneHvacRefrigerationChillerSet(DataObject):
    ...
    """
    _schema = {'extensible-fields': OrderedDict([(u'air chiller 1 name', {'name': u'Air Chiller 1 Name', 'pyname': u'air_chiller_1_name', 'required-field': False, 'autosizable': False, 'autocalculatable': False, 'type': u'object-list'})]),
 ...
 'unique-object': False}
    ...
    def add_extensible(self,
                       air_chiller_1_name=None,
                       ):
        """ Add values for extensible fields

        Args:

            air_chiller_1_name (str): value for IDD Field `Air Chiller 1 Name`
                if `value` is None it will not be checked against the
                specification and is assumed to be a missing value
        """
        vals = []
        air_chiller_1_name = self.check_value("Air Chiller 1 Name", air_chiller_1_name)
        vals.append(air_chiller_1_name)
        self._extdata.append(vals)
